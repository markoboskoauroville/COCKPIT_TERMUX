...
Copied verbatim from KEYRING_TERMUX/localguard.py (13.9.2026), the header name changed. The cockpit
can force-stop apps and change system settings through the ADB bridge, so all three checks apply
to every /api/ call, GET included: a page open in another tab must not be able to drive it.
...
OPEN_ENDPOINTS = {"static", "favicon", "favicon_ico", "index", "health"}
